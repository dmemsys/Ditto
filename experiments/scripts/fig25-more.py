...
from utils.plots import plot_fig25_more
...
for cache_size in cache_size_list:
    print(
        f"Start executing sample-adaptive with {cache_size} freq_cache under {workload}")
    ...
    for j in range(num_avg):
        controller_prom = cmd_manager.execute_on_node(
            master_id, f"cd {work_dir} && ./run_controller.sh sample-adaptive 1 256 {workload}")
        ...
        for i in range(8):
            ...
            c_prom = cmd_manager.execute_on_node(
                client_ids[i], f"cd {work_dir} && ./run_client_master.sh {method} {st_cid} {workload} 32 256")
...
save_res(f'fig25-{workload}', all_res)

plot_res = load_res(f'fig25-{workload}.json')
plot_fig25_more(plot_res, workload)

et = time.time()
save_time(f'fig25-{workload}', et - st)
