...
from utils.plots import plot_fig24_more
...
MAKE_CMD = get_make_cmd(build_dir, "sample-adaptive",
                        workload, "0.1", running_opt)
cmd_manager.execute_all(MAKE_CMD)
CACHE_CONFIG_CMD = get_cache_config_cmd(config_dir, workload, "0.1")
...
for method in method_list:
    print(f"Start executing {method} on {workload} with 0.1 cache size")
    tmp_res = {'tpt': 0}

    j = 0
    while j < num_avg:
        controller_prom = cmd_manager.execute_on_node(
            master_id, f"cd {work_dir} && ./run_controller.sh {method} 1 64 {workload}")
        ...
        for i in range(2):
            ...
            c_prom = cmd_manager.execute_on_node(
                client_ids[i], f"cd {work_dir} && ./run_client_master.sh {method} {st_cid} {workload} 32 64")
...
# save res
save_res(f"fig24-{workload}", all_res)

plot_fig24_more(all_res, workload)

et = time.time()
save_time(f"fig24-{workload}", et - st)
